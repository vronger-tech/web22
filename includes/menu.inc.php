...
Class Menu {
    ...
    public static function getMenu($sItems) {
        ...
        {
            if($menuitem[1] == "")
            { $menu.= "<a href='".SITE_ROOT.$menuindex."' ".($menuindex==$sItems[0]? "class='selected'":"").">".$menuitem[0]."</a>"; }
            else if($menuitem[1] == $sItems[0])
            { $submenu .= "<a href='".SITE_ROOT.$sItems[0]."/".$menuindex."' ".($menuindex==$sItems[1]? "class='selected'":"").">".$menuitem[0]."</a>"; }
        }
        $menu.="</ul>";
